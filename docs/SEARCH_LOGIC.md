...
### 📌 High-Level System Diagram (상위 시스템 구조도)

```
┌─────────────────────────────┐
│        User Interface        │
│ (Search bar, Button, Render) │
└───────────────┬─────────────┘
                │
                ▼
┌───────────────────────────────────┐
│    Search Manager (Main Logic)    │
│  - Validation                     │
│  - Keyword handling               │
│  - Router of entire workflow      │
└───────────────┬──────────────────┘
                │
        ┌───────┴────────┬──────────┐
        ▼                 ▼          ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────────┐
│ Local Cache   │  │ Supabase DB  │  │ YouTube API (v3) │
│ (Backup only) │  │ (Primary)    │  │ (Only if needed) │
└───────┬──────┘  └──────┬───────┘  └─────────┬────────┘
        │                │                    │
        │                ▼                    │
        │        Cached Search Data           │
...
│ - Video list                │
│ - Counts                    │
│ - Status (API skipped etc.)│
└─────────────────────────────┘
```

### 🟦 Detailed Step-by-Step Flow Diagram (세부 단계 흐름도)
...
[2] Initial Validation
 ...
 - Check login + API key
        │
        ▼
[3] Local Cache Check (backup only)
 - Not used for API logic
 - Only used if server fails
        │
        ▼
[4] Query Supabase (Primary Source)
        │
        ├───▶ If Supabase fails:
        │        ├─ Has Local Cache → Use it + Stop
        │        └─ No Local Cache → Error + Stop
        │
        ▼
[5] Supabase Data Found?
        │
        ├──▶ NO:
        │        ▼
        │    [A] No Server Data Case
        │        - Render local cache if exists (optional)
        │        - Perform Full YouTube Search
        │        - Save to Supabase
        │        - Sync Local Cache
        │        ▼
        │      Done
        ...
        └──▶ YES:
                 ▼
         [B] Server Data Exists Case
                 │
                 ▼
        [6] Data Consistency Check
         - If total_count ≠ actual_count → Auto fix
                 │
                 ▼
        [7] Enough Data Check (Highest Priority)
         - If supabaseCount ≥ 200:
                 ├─ Restore server data
                 ├─ Sync local cache
                 ├─ Render page
                 ├─ Background NULL updater
                 └─ **Skip YouTube API completely**
                 ▼
                Done
                 │
                 ▼
        [8] Additional Data Needed?
         - neededCount = min( targetCount - supabaseCount,
                               MAX_RESULTS_LIMIT - supabaseCount )
                 │
                 ▼
        [9] Additional Fetch (Missing Only)
         - Exclude existing videoIds
         - Query YouTube for missing ones
         - Save & dedupe
         - Update server
         - Sync local cache
         ▼
...
### 🟩 System Behavior Summary Diagram (시스템 결정 로직 요약도)
...
```
                ...
                └─────────────┬──────────┘
                    ...
                    └──────────────────┘
                              ...
                              ▼
         ┌────────────────────────────────────────┐
         │ neededCount = 200 - supabaseCount      │
         │ Additional YouTube Fetch (Missing only) │
         └────────────────────────────────────────┘
```

### 🟦 Functional Architecture (Component View) (기능별 구성도)
